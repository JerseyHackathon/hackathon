...
export default function AiChef({ apiKey, onUpdateMeals, food}){
  ...
  const generateText = () => {
    ...
    const foodNames = food.map(ele=>ele.food_name)
    if(!food){
           ...
           "messages": [
              {
                ...
                "content": `Return to me multiple meals you can make with these ingredients: ${foodNames}:`
              }
            ],
              "max_tokens": 1000
          }, {
    ...
    //   onUpdateMeals(gptResponse); //update input value in timeline AKa corrects tweet typed
  ...
  return (
    <div>
        <Button onClick={generateText}> Ai Chef
